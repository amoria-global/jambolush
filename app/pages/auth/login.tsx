...
function LoginContent() {
  ...
  // Function to handle step 1 (email check)
  const handleEmailSubmit = async () => {
    if (!email) {
      setNotify({type: "warning", message: "Please enter your email address."});
      return;
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
      setNotify({type: "warning", message: "Please enter a valid email address."});
      return;
    }
    ...
    try {
      ...
      if (!userStatus.success || !userStatus.data?.exists) {
        setNotify({type: "error", message: "No account found with this email address. Please sign up first."});
        setLoading(false);
        return;
      }
      ...
      if (!userData.hasPassword) {
        // User exists but has no password - redirect to reset password
      ...
      setNotify({type: "error", message: "An error occurred. Please try again."});
      setLoading(false);
    }
  };

  // Function to handle step 2 (password login)
  const handlePasswordSubmit = async () => {
    ...
    if (password.length < 6) {
      setNotify({type: "warning", message: "Password must be at least 6 characters long."});
      return;
    }
    ...
    try {
      ...
      // UPDATED: Redirect to reset-password instead of set-password
      ...
      if (errorMessage.includes('Please set up your password first') || errorMessage.includes('Check your email for instructions')) {
        try {
          ...
          setTimeout(() => {
            const currentRedirect = searchParams.get('redirect') || searchParams.get('returnUrl') || searchParams.get('return_to');
            const resetPasswordUrl = currentRedirect 
              ? `/all/reset-password?redirect=${encodeURIComponent(currentRedirect)}` 
              : `/all/reset-password`;
            router.push(resetPasswordUrl);
          }, 2000);
        }
  ...
  useEffect(() => {
    ...
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!showPasswordField) {
      // Step 1: Only email validation
      setIsFormValid(emailRegex.test(email));
    } else {
      // Step 2: Email and password validation
  ...
  // UPDATED handleUserRedirect function - REMOVED password null check since we handle it before login
  const handleUserRedirect = (userData: any, token?: string, refreshToken?: string) => {
    ...
    // Condition 2: Verified but not active -> Redirect to reset password
          ...
          <div className="max-w-sm mx-auto">
            ...
            {showPasswordField && (
              <button
                ...
                className="flex items-center text-blue-300 hover:text-blue-200 text-sm mb-4 transition-colors duration-200"
              >
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2.5 backdrop-blur-sm border rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-1 focus:ring-white/20 text-sm ${
                    showPasswordField 
                      ? 'bg-white/5 border-white/10 cursor-not-allowed' 
                      : 'bg-white/10 border-white/30 focus:border-white/60'
                  }`}
                  required
                />
              </div>
              ...
              >
                {loading 
                  ? (!showPasswordField ? 'Checking...' : 'Signing In...') 
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/20 rounded-3xl p-6 lg:p-8 shadow-2xl">
            ...
            {showPasswordField && (
              <button
                onClick={handleBackToEmail}
                className="flex items-center text-blue-300 hover:text-blue-200 text-sm mb-6 transition-colors duration-200"
              >
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Enter your email"
                    disabled={showPasswordField}
                    className={`w-full px-4 py-2.5 lg:py-3 backdrop-blur-sm border rounded-xl text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/20 transition-all duration-300 text-sm lg:text-base ${
                      showPasswordField 
                        ? 'bg-white/5 border-white/10 cursor-not-allowed' 
                        : 'bg-white/10 border-white/30 focus:border-white/60 focus:bg-white/15'
                    }`}
                    required
                  />
                </div>
