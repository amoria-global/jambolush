...
function SignupPage({
  ...
  const handleGoogleCallback = async (response: any) => {
    ...
    try {
      ...
      if (result.data?.accessToken) {
        ...
        setNotify({type: "success", message: t('auth.signupSuccess')});
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data?.message === 'User already exists' || response.data?.error || (response.data?.message && !response.data?.accessToken)) {
        throw { status: 409, data: response.data };
      }

      if (response.data?.accessToken || response.status === 200 || response.status === 201) {
        setNotify({type: "success", message: `${t('auth.signupSuccess')}! Redirecting...`});
        setTimeout(() => router.push('/all/login'), 2000);
      } else {
        throw { status: response.status || 500, data: response.data || { message: "Unexpected response format" } };
      }
      ...
      if (error.status) {
        ...
        if (data?.message) {
          ...
          switch (status) {
            case 400:
              setNotify({type: "error", message: t('auth.invalidCredentials')});
              break;
            case 409:
            ...
            case 422:
              ...
              break;
            case 500:
              setNotify({type: "error", message: t('error.serverError')});
              break;
            default:
              setNotify({type: "error", message: `${t('common.error')} (Error ${status})`});
          }
        }
      } else if (error.name === 'AbortError' || error.message?.includes('timeout')) {
        setNotify({type: "error", message: t('error.networkError')});
      } else {
        const errorMessage = error.message || t('error.somethingWentWrong');
  ...
  const getCountryCode = (countryName: string): string => {
    const countryCodes: { [key: string]: string } = {
      ...
      'Germany': 'DE', 'France': 'FR', 'Spain': 'ES', 'Italy': 'IT', 'Japan': 'JP',
      'China': 'CN', 'India': 'IN', 'Brazil': 'BR', 'Mexico': 'MX', 'Rwanda': 'RW',
  ...
  if (isMobileView) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0B1426] via-[#0F1B35] to-[#083A85] flex flex-col relative font-['Inter',sans-serif] antialiased">
        {notify && (
          <AlertNotification message={notify.message} type={notify.type} position="top-center" duration={5000} size="md" showProgress={true} autoHide={true} onClose={handleNotificationClose} />
        )}
        <div ref={langDropdownRef} className="absolute top-4 right-4 z-20">
          ...
          {isLangOpen && (
            <div className="absolute top-full right-0 mt-2 w-32 bg-white/10 backdrop-blur-xl border border-white/20 rounded-lg shadow-xl py-1">
              {languages.map((lang) => (
                <button key={lang.code} onClick={() => { changeLanguage(lang.code); setIsLangOpen(false); }} className="w-full text-left px-3 py-2 text-base text-white hover:bg-white/20 flex items-center space-x-2 transition-colors duration-200">
                  <span>{lang.flag}</span>
                  <span>{lang.name}</span>
                </button>
              ))}
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder={t('forms.firstName')} className="w-full px-3 py-2.5 bg-white/10 border border-white/30 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-1 focus:ring-white/20 text-base" required />
                        </div>
                        <div>
                            <label className="block text-white/90 text-base font-medium mb-1.5">{t('forms.lastName')}</label>
                            <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder={t('forms.lastName')} className="w-full px-3 py-2.5 bg-white/10 border border-white/30 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-1 focus:ring-white/20 text-base" required />
                        </div>
                    </div>
                    <div>
                        <label className="block text-white/90 text-base font-medium mb-1.5">{t('forms.email')}</label>
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t('forms.email')} className="w-full px-3 py-2.5 bg-white/10 border border-white/30 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-1 focus:ring-white/20 text-base" required />
                    </div>
                    <div>
                        <label className="block text-white/90 text-base font-medium mb-1.5">{t('forms.phone')}</label>
                        <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder={t('forms.phone')} className="w-full px-3 py-2.5 bg-white/10 border border-white/30 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-1 focus:ring-white/20 text-base" />
                    </div>
                    <div>
                        <label className="block text-white/90 text-base font-medium mb-1.5">{t('forms.country')}</label>
                        <input type="text" value={country} onChange={(e) => setCountry(e.target.value)} placeholder={t('forms.country')} className="w-full px-3 py-2.5 bg-white/10 border border-white/30 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-1 focus:ring-white/20 text-base" />
                    </div>
                    <div>
                        <label className="block text-white/90 text-base font-medium mb-1.5">{t('forms.password')}</label>
                        <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t('auth.createPassword')} className="w-full px-3 py-2.5 bg-white/10 border border-white/30 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-1 focus:ring-white/20 text-base" required />
                    </div>
                    <div>
                        <label className="block text-white/90 text-base font-medium mb-1.5">{t('forms.confirmPassword')}</label>
                        <input type={showConfirmPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder={t('auth.confirmYourPassword')} className="w-full px-3 py-2.5 bg-white/10 border border-white/30 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-1 focus:ring-white/20 text-base" required />
                         {password && confirmPassword && password !== confirmPassword && (
                            <p className="text-red-300 text-base mt-1">{t('forms.passwordsDoNotMatch')}</p>
                        )}
                    </div>
                    <div className="flex items-start space-x-2">
                        ...
                        <label htmlFor="terms-mobile" className="text-white/80 text-base">{t('auth.agreeToTerms')} <button className="text-blue-300 underline">{t('auth.termsOfService')}</button> {t('auth.and')} <button className="text-blue-300 underline">{t('auth.privacyPolicy')}</button></label>
                    </div>
                    ...
                    <div className="relative my-4">
                      <div className="flex items-center">
                        ...
                        <span className="px-3 text-white/60 text-base">{t('auth.orSignUpWith')}</span>
                        <div className="flex-1 border-t border-white/20"></div>
                      </div>
                    </div>
                    <button type="button" onClick={handleGoogleSignIn} disabled={googleLoading} className="w-full bg-white hover:bg-gray-50 border border-gray-200 text-gray-700 py-2.5 px-4 rounded-lg flex items-center justify-center space-x-3 transition-all duration-300">
                        ...
                        <span className="font-medium">{googleLoading ? t('auth.signingUp') : t('auth.signUpWithGoogle')}</span>
                    </button>
                </div>
  ...
  return (
    <div className="min-h-screen flex font-['Inter',sans-serif] antialiased">
      {notify && ( <AlertNotification message={notify.message} type={notify.type} position="top-center" duration={5000} size="md" showProgress={true} autoHide={true} onClose={handleNotificationClose}/> )}
      <div className="hidden md:flex flex-1 bg-gradient-to-br from-[#0B1426] via-[#0F1B35] to-[#0B1426] items-center justify-center p-4 min-h-screen relative overflow-hidden">
        ...
        <div className="text-center relative z-10">
          ...
          <div className="space-y-2">
            ...
            <p className="text-white/60 text-base mt-4 max-w-sm mx-auto leading-relaxed">{t('about.hero.description')}</p>
          </div>
        </div>
      </div>

      {/* Right Side - Signup Form -- FIX: Added overflow-y-auto */}
        ...
        <div className="w-full max-w-md relative z-10 py-8">
          <div className="bg-white/5 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label htmlFor="firstName" className="block text-white/90 text-base font-medium">{t('forms.firstName')}</label>
                  <input type="text" id="firstName" name="firstName" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder={t('forms.firstName')} className="w-full px-4 py-3 bg-white/10 backdrop-blur-sm border border-white/30 rounded-xl text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-2 focus:ring-white/20 focus:bg-white/15 transition-all duration-300" required/>
                </div>
                <div className="space-y-2">
                  <label htmlFor="lastName" className="block text-white/90 text-base font-medium">{t('forms.lastName')}</label>
                  <input type="text" id="lastName" name="lastName" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder={t('forms.lastName')} className="w-full px-4 py-3 bg-white/10 backdrop-blur-sm border border-white/30 rounded-xl text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-2 focus:ring-white/20 focus:bg-white/15 transition-all duration-300" required/>
                </div>
              </div>
              <div className="space-y-2">
                <label htmlFor="email" className="block text-white/90 text-base font-medium">{t('auth.emailAddress')}</label>
              ...
              <div className="space-y-2">
                <label htmlFor="password" className="block text-white/90 text-base font-medium">{t('auth.password')}</label>
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} id="password" name="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={`${t('auth.createPassword')} (${t('forms.passwordTooShort')})`} className="w-full px-4 py-3 pr-12 bg-white/10 backdrop-blur-sm border border-white/30 rounded-xl text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-2 focus:ring-white/20 focus:bg-white/15 transition-all duration-300" required/>
              ...
              <div className="space-y-2">
                <label htmlFor="confirmPassword" className="block text-white/90 text-base font-medium">{t('forms.confirmPassword')}</label>
                <div className="relative">
                  <input type={showConfirmPassword ? "text" : "password"} id="confirmPassword" name="confirmPassword" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder={t('auth.confirmYourPassword')} className="w-full px-4 py-3 pr-12 bg-white/10 backdrop-blur-sm border border-white/30 rounded-xl text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:ring-2 focus:ring-white/20 focus:bg-white/15 transition-all duration-300" required/>
                  <button type="button" onClick={toggleConfirmPasswordVisibility} aria-label={showConfirmPassword ? "Hide password" : "Show password"} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-white/60 hover:text-white/90 focus:outline-none transition-colors duration-200 cursor-pointer">
                    {showConfirmPassword ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878L21 21" /></svg> : <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>}
                  </button>
                </div>
                {password && confirmPassword && password !== confirmPassword && ( <p className="text-red-300 text-base mt-1">{t('forms.passwordsDoNotMatch')}</p> )}
              </div>
              <div className="flex items-start space-x-3">
                <input type="checkbox" id="terms" name="terms" checked={agreeToTerms} onChange={(e) => setAgreeToTerms(e.target.checked)} className="w-4 h-4 mt-0.5 text-blue-400 bg-white/10 border-white/30 rounded focus:ring-blue-400 focus:ring-2 cursor-pointer"/>
                <label htmlFor="terms" className="text-white/80 text-base cursor-pointer select-none leading-relaxed">{t('auth.agreeToTerms')}{' '}<button type="button" className="text-blue-300 hover:text-blue-200 underline">{t('auth.termsOfService')}</button>{' '}{t('auth.and')}{' '}<button type="button" className="text-blue-300 hover:text-blue-200 underline">{t('auth.privacyPolicy')}</button></label>
              </div>
              <button type="submit" disabled={!isFormValid || loading} className={`w-full py-3 px-6 rounded-xl font-semibold transition-all duration-300 shadow-lg transform ${ isFormValid && !loading ? 'bg-gradient-to-r from-white to-gray-100 text-gray-800 hover:from-gray-100 hover:to-white hover:scale-[1.02] hover:shadow-xl cursor-pointer' : 'bg-gray-400 text-gray-600 cursor-not-allowed opacity-60' }`}>
                {loading ? t('auth.creatingAccount') : t('auth.createAccount')}
              </button>
            </form>
            <div className="text-center mt-6">
              <span className="text-white/70 text-base">{t('auth.alreadyHaveAccount')} </span>
              <a href="/all/login" className="group text-blue-300 text-base hover:text-blue-200 font-medium transition-all duration-200 cursor-pointer relative">
            ...
            <div className="relative my-6">
              <div className="flex items-center">
                <div className="flex-1 border-t border-white/20"></div>
                <span className="px-4 text-white/60 text-base">{t('auth.orSignUpWith')}</span>
                <div className="flex-1 border-t border-white/20"></div>
              </div>
            </div>
            <button type="button" onClick={handleGoogleSignIn} disabled={googleLoading} aria-label={t('auth.signUpWithGoogle')} className="group w-full bg-white hover:bg-gray-50 border border-gray-200 hover:border-gray-300 text-gray-700 py-3 px-4 rounded-xl font-medium transition-all duration-300 flex items-center justify-center space-x-3 transform hover:scale-105 shadow-sm hover:shadow-md cursor-pointer">
              <svg className="w-5 h-5" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
              <span>{googleLoading ? t('auth.signingUp') : t('auth.signUpWithGoogle')}</span>
            </button>
          </div>
        </div>
      </div>
